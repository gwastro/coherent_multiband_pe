...
for seed in {21..70}; do
    output_dir="./output_multiband_seed_${seed}_obs_3.0yrs_wait_5yrs_nessai"
    ...
    if [ -d "$output_dir" ]; then
        ...
        if [ -z "$(ls -A "$output_dir")" ]; then
            ...
            --injection-file-folder "/work/shichao.wu/Area51/coherent_multiband_prod/population_run/multiverse_seed_${seed}/injection_above_snr_threshold_seed_${seed}_obs_3.0yrs_wait_5.0yrs/" \
            --injection-snr-duration-file "snr_seed_${seed}_obs_3.0yrs_wait_5.0yrs.csv" \
            --lisa-obs-duration 3.0 \
            --inference-config prior_multiband.ini \
            --config-files workflow_multiband.ini \
            --workflow-name "multiband_workflow_seed_${seed}_obs_3.0yrs_wait_5yrs_nessai" \
